...
import type { CreateSaverInput, UpdateSaverInput, PatchSaverInput } from './validation';

// Saver API endpoints
export const saversApi = {
  // Get all savers with pagination
  getAll: (params: PaginationParams) =>
    api.getPaginated<Saver>('/api/savers', params as PaginationParams & Record<string, unknown>),

  // Get saver by ID
  getById: (id: string) =>
    api.get<Saver>(`/api/savers/${id}`),

  // Create new saver
  create: (data: CreateSaverInput) =>
    api.post<Saver>('/api/savers', data),
  ...
  // Get saver loans
  getLoans: (id: string) =>
    api.get<SaverLoan[]>(`/api/savers/${id}/loans`),
  ...
  // Terminate contract
  terminate: (id: string, data: { month: number; year: number; terminationReason?: string }) =>
