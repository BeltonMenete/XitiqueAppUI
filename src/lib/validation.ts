...
const ResetPasswordSchema = v.object({
	password: v.pipe(
		...
		v.regex(/[A-Z]/, 'A palavra-passe deve conter pelo menos uma letra maiúscula.'),
		v.regex(/[0-9]/, 'A palavra-passe deve conter pelo menos um número.'),
		v.regex(/[!@#$%^&*(),.?":{}|<>]/, 'A palavra-passe deve conter pelo menos um carácter especial.'),
	),
...
export function validateResetPasswordForm(password: string, confirmPassword: string): { valid: boolean; errors: Record<string, string> } {
