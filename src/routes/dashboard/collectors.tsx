          ...
          <div className="relative">
            ...
            <span className={cn(
              ...
              row.status === 'active' ? 'bg-emerald-500' :
                row.status === 'suspended' ? 'bg-red-500' : 'bg-slate-400'
            )} />
          </div>
          <div>
    ...
    },
    {
      ...
      render: (value: unknown) => (
        <div className="flex items-center gap-1">
          <span className="font-bold text-sm text-slate-900">{String(value)}</span>
    ...
    },
    {
      ...
      render: (value: unknown) => (
        <span className={cn(
          ...
          Number(value) > 0 ? 'text-emerald-600' :
            Number(value) < 0 ? 'text-red-600' : 'text-slate-500'
        )}>
          {Number(value) > 0 ? '+' : ''}{Number(value).toLocaleString()} MZN
        </span>
      ),
    },
    {
      ...
      render: (_: unknown, row: Collector) => {
        ...
        if (row.status === 'suspended') return <PendingBadge>Em Análise</PendingBadge>;
