...
export function QuickLoanModal({ isOpen, onClose, onSubmit, saverName, maxLoanAmount = 50000 }: QuickLoanModalProps) {
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={handleCancel} title="Solicitar Empréstimo" size="md">
        <form onSubmit={handleSubmit} className="space-y-4">
          {saverName && (
            <div className="bg-slate-50 p-3 rounded-lg">
              <p className="text-sm text-slate-600">Empréstimo para: <span className="font-semibold text-slate-900">{saverName}</span></p>
            </div>
          )}
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">Máximo disponível: {maxLoanAmount.toLocaleString()} MZN</p>
          </div>
          ...
          <div>
            <label htmlFor="repaymentDate" className="block text-sm font-medium text-slate-700 mb-1">
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <label htmlFor="isEmergency" className="flex items-center gap-2 text-sm text-slate-700">
          ...
          {formData.isEmergency && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
              <p className="text-xs text-amber-800">
                ⚠️ Empréstimos de emergência são processados prioritariamente mas podem ter taxas mais elevadas.
              </p>
            </div>
          )}
      ...
      {showToast && (
        <div className="fixed bottom-6 right-6 bg-white border-l-4 border-emerald-500 shadow-lg rounded-lg p-4 transform transition-all duration-300 z-50 flex items-start gap-4 max-w-sm">
          ...
          <div className="flex-1">
            <h4 className="text-sm font-medium text-slate-900">Solicitação Enviada</h4>
            <p className="text-xs text-slate-500 mt-1">O empréstimo será analisado e você receberá uma resposta breve.</p>
          </div>
        </div>
      )}
    </>
