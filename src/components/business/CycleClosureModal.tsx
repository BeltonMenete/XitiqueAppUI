...
export function CycleClosureModal({ isOpen, onClose, onSubmit, currentMonth = 5, currentYear = 2024 }: CycleClosureModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Fechar Ciclo Mensal" size="md">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
          <div className="flex items-start gap-3">
            <AlertTriangle className="text-amber-600 mt-0.5" size={20} />
            <div>
              <p className="text-sm font-semibold text-amber-900">Ação Irreversível</p>
              <p className="text-xs text-amber-700">
                Ao fechar o ciclo, os pagamentos serão processados e o mês será encerrado para todos os ticantes.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={(e) => setFormData({ ...formData, processPayouts: e.target.checked })}
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={(e) => setFormData({ ...formData, rolloverActiveSavers: e.target.checked })}
              className="w-4 h-4 text-emerald-600 rounded focus:ring-2 focus:ring-emerald-500/20"
            />
            <label htmlFor="rolloverActiveSavers" className="text-sm text-slate-700">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="w-4 h-4 text-emerald-600 rounded focus:ring-2 focus:ring-emerald-500/20"
            />
            <label htmlFor="sendNotifications" className="text-sm text-slate-700">
          ...
          <CardContent className="p-4">
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-emerald-600">450.000 MZN</span>
              </div>
              <div className="flex justify-between">
        ...
        <ModalFooter>
          ...
          <Button leftIcon={<RefreshCw size={16} />}>
            Fechar Ciclo
          </Button>
        </ModalFooter>
      </form>
