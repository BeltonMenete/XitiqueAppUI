import { Calendar, DollarSign, Edit, Trash2, History, AlertCircle, CheckCircle, FileText, Clock, MoreHorizontal, X } from "lucide-react";
import { useState } from "react";
import { Button } from "#/components/ui/Button";
import { Card, CardContent } from "#/components/ui/Card";
import { Modal } from "#/components/ui/Modal";
import { ContextMenu, ContextMenuItem, ContextMenuSeparator, ContextMenuHeader } from "#/components/ui/ContextMenu";
...
export function DayActionModal({
	...
	const getActionsForStatus = (status: string) => {
		switch (status) {
			case "unpaid":
				return [
					{ label: "Registrar Depósito", icon: <DollarSign size={16} />, action: "deposit" },
					{ label: "Marcar Não Disponível", icon: <Clock size={16} />, action: "unavailable" },
					{ label: "Adicionar Nota", icon: <FileText size={16} />, action: "note" },
				];
			case "paid":
				return [
					{ label: "Ver Detalhes", icon: <FileText size={16} />, action: "details" },
					{ label: "Editar Depósito", icon: <Edit size={16} />, action: "edit" },
					{ label: "Deletar Depósito", icon: <Trash2 size={16} />, action: "delete", danger: true },
					{ label: "Converter para Pagamento de Dívida", icon: <CheckCircle size={16} />, action: "convert" },
					{ label: "Ver Recibo", icon: <FileText size={16} />, action: "receipt" },
				];
			case "debt":
				return [
					{ label: "Registrar Pagamento de Dívida", icon: <DollarSign size={16} />, action: "debt_payment" },
					{ label: "Ver Detalhes da Dívida", icon: <AlertCircle size={16} />, action: "debt_details" },
					{ label: "Calcular Plano de Pagamento", icon: <Calendar size={16} />, action: "payment_plan" },
				];
			case "debt_payment":
				return [
					{ label: "Ver Detalhes do Pagamento", icon: <FileText size={16} />, action: "payment_details" },
					{ label: "Editar Pagamento", icon: <Edit size={16} />, action: "edit_payment" },
					{ label: "Reverter para Depósito Normal", icon: <History size={16} />, action: "revert" },
					{ label: "Ver Histórico do Empréstimo", icon: <History size={16} />, action: "loan_history" },
				];
			default:
				return [];
		}
	...
	const handleConvertConfirm = () => {
		console.log(`Convert deposit to debt payment for ${saverName} on day ${day}`);
		onActionComplete?.("convert", { day, saverName });
		setShowConfirmConvert(false);
	};

	const handleUnavailableSubmit = () => {
		console.log(`Mark day ${day} as unavailable for ${saverName}: ${unavailableReason}`);
		onActionComplete?.("unavailable", { day, saverName, reason: unavailableReason });
	...
	const getStatusColor = (status: string) => {
		switch (status) {
			case "paid":
				return isDebtPayment ? "bg-amber-50 border-amber-200" : "bg-emerald-50 border-emerald-200";
			case "unpaid":
	...
	return (
		<>
			<Modal isOpen={isOpen} onClose={onClose} title={`Ações para Dia ${day}`} size="md">
				<Card className="border-0 shadow-none">
					<CardContent className="p-6 space-y-6">
						{/* Day Information */}
						<div className={cn("p-4 rounded-lg border", getStatusColor(dayStatus))}>
							<div className="flex items-center justify-between mb-2">
								...
								<span className={cn(
									...
									dayStatus === "paid" && !isDebtPayment && "bg-emerald-100 text-emerald-700",
									dayStatus === "paid" && isDebtPayment && "bg-amber-100 text-amber-700",
							...
							{amount && dayStatus === "paid" && (
								<div className="flex items-center justify-between text-sm">
									...
									<span className="font-medium text-slate-900">{amount.toLocaleString()} MZN</span>
								</div>
							)}
						...
						{showDepositForm && (
							<div className="p-4 bg-slate-50 rounded-lg space-y-4">
								<h4 className="text-sm font-semibold text-slate-900">
									{dayStatus === "unpaid" ? "Registrar Depósito" : "Editar Depósito"}
								</h4>
						...
						{showNoteForm && (
							<div className="p-4 bg-slate-50 rounded-lg space-y-4">
								<h4 className="text-sm font-semibold text-slate-900">Adicionar Nota</h4>
								<div>
						...
						{showUnavailableForm && (
							<div className="p-4 bg-slate-50 rounded-lg space-y-4">
								<h4 className="text-sm font-semibold text-slate-900">Marcar como Não Disponível</h4>
								<div>
								...
								{unavailableReason === "Outro" && (
									<div className="mt-2">
										<label className="block text-xs text-slate-600 mb-1">Especificar</label>
										<input
											type="text"
											value={noteText}
											onChange={(e) => setNoteText(e.target.value)}
								...
								<div className="flex gap-2">
									...
									<Button size="sm" variant="outline" onClick={() => setShowUnavailableForm(false)}>
										Cancelar
									</Button>
								</div>
							</div>
						...
						{showConfirmDelete && (
							<div className="p-4 bg-red-50 border border-red-200 rounded-lg space-y-4">
								<h4 className="text-sm font-semibold text-red-900">Confirmar Exclusão</h4>
								<p className="text-sm text-red-700">
									Tem certeza que deseja deletar o depósito de {amount?.toLocaleString()} MZN para {saverName} no dia {day}?
								</p>
								<div className="flex gap-2">
						...
						{showConfirmConvert && (
							<div className="p-4 bg-amber-50 border border-amber-200 rounded-lg space-y-4">
								<h4 className="text-sm font-semibold text-amber-900">Converter para Pagamento de Dívida</h4>
								<p className="text-sm text-amber-700">
									Tem certeza que deseja converter este depósito de {amount?.toLocaleString()} MZN em pagamento de dívida?
								</p>
								<div className="flex gap-2">
									<Button size="sm" onClick={handleConvertConfirm}>
										Confirmar Conversão
									</Button>
									<Button size="sm" variant="outline" onClick={() => setShowConfirmConvert(false)}>
										Cancelar
									</Button>
								</div>
							</div>
						...
						{/* Primary Action */}
						{!showDepositForm && !showNoteForm && !showConfirmDelete && !showConfirmConvert && !showUnavailableForm && (
							<div className="space-y-3">
								<Button
									size="lg"
									className="w-full"
									leftIcon={
										dayStatus === "unpaid" ? <DollarSign size={20} /> :
											dayStatus === "paid" ? <FileText size={20} /> :
												dayStatus === "debt" ? <DollarSign size={20} /> :
													<FileText size={20} />
									}
									onClick={() => {
								...
								{actions.length > 1 && (
									<div className="relative">
										...
										{showMenu && menuPosition && (
											...
											>
												<ContextMenuHeader>Opções para Dia {day}</ContextMenuHeader>
